...
type QueuedCallsHandler struct {
	pg    storage.PostgresStore
	ch    storage.ClickHouseStore
	redis storage.RedisCache
}

func NewQueuedCallsHandler(pg storage.PostgresStore, ch storage.ClickHouseStore, redis storage.RedisCache) *QueuedCallsHandler {
	return &QueuedCallsHandler{pg: pg, ch: ch, redis: redis}
}

func (h *QueuedCallsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	data, err := getOrComputeQueuedCalls(r.Context(), h.redis, tenantID, jobID.String())
	if err != nil {
		api.Error(w, http.StatusInternalServerError, api.ErrCodeInternalError, "queued calls data not available")
		return
	}
