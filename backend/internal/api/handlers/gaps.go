...
func (h *GapsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	cacheKey := h.redis.TenantKey(tenantID, "gaps", jobID.String())
	...
	data, err := h.ch.GetGaps(r.Context(), tenantID, jobID.String())
	if err != nil {
		slog.Error("failed to retrieve gaps", "job_id", jobID, "error", err)
		api.Error(w, http.StatusInternalServerError, api.ErrCodeInternalError, "failed to retrieve gaps data")
		return
	}
