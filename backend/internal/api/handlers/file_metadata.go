...
func NewFileMetadataHandler(pg storage.PostgresStore, ch storage.ClickHouseStore, redis storage.RedisCache) *FileMetadataHandler {
	return &FileMetadataHandler{pg: pg, ch: ch, redis: redis}
}
...
func (h *FileMetadataHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	data, err := getOrComputeFileMetadata(r.Context(), h.redis, tenantID, jobID.String())
	if err != nil {
		api.Error(w, http.StatusInternalServerError, api.ErrCodeInternalError, "file metadata not available")
		return
	}
