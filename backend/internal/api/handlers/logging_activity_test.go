...
func TestLoggingActivityHandler_ServeHTTP(t *testing.T) {
	...
	sampleResponse := &domain.LoggingActivityResponse{
		JobID: jobID.String(),
		Activities: []domain.LoggingActivity{
			{
				LogType:        "API",
				FirstTimestamp: time.Date(2026, 2, 3, 10, 0, 0, 0, time.UTC),
				LastTimestamp:  time.Date(2026, 2, 3, 18, 30, 45, 0, time.UTC),
				DurationMS:     30645000,
			},
			{
				LogType:        "SQL",
				FirstTimestamp: time.Date(2026, 2, 3, 10, 0, 1, 0, time.UTC),
				LastTimestamp:  time.Date(2026, 2, 3, 18, 30, 42, 0, time.UTC),
				DurationMS:     30641000,
			},
		},
	}
	...
	tests := []struct {
		...
		{
			name:     "cache_hit_returns_200_with_activities",
			...
			setupMocks: func(pg *testutil.MockPostgresStore, ch *testutil.MockClickHouseStore, redis *testutil.MockRedisCache) {
				...
				redis.On("Get", mock.Anything, baseKey+":logging-activity").Return(string(cachedJSON), nil)
			},
			expectedStatus: http.StatusOK,
			checkBody: func(t *testing.T, body []byte) {
				var resp domain.LoggingActivityResponse
				require.NoError(t, json.Unmarshal(body, &resp))
				assert.Len(t, resp.Activities, 2)
				assert.Equal(t, "API", resp.Activities[0].LogType)
				assert.Equal(t, "SQL", resp.Activities[1].LogType)
			},
		},
		{
			...
			setupMocks: func(pg *testutil.MockPostgresStore, ch *testutil.MockClickHouseStore, redis *testutil.MockRedisCache) {
				...
				baseKey := fmt.Sprintf("tenant:%s:dashboard:%s", tenantID.String(), jobID.String())
				redis.On("TenantKey", tenantID.String(), "dashboard", jobID.String()).Return(baseKey)
				redis.On("Get", mock.Anything, baseKey+":logging-activity").Return("", fmt.Errorf("cache miss"))
			},
			expectedStatus: http.StatusOK,
			checkBody: func(t *testing.T, body []byte) {
				var resp domain.LoggingActivityResponse
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			handler := NewLoggingActivityHandler(pg, ch, redis)

			req := httptest.NewRequest(http.MethodGet, "/api/v1/analysis/"+tc.jobIDStr+"/dashboard/logging-activity", nil)
			if tc.tenantID != "" {
