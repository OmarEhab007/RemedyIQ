...
type LoggingActivityHandler struct {
	pg    storage.PostgresStore
	ch    storage.ClickHouseStore
	redis storage.RedisCache
}

func NewLoggingActivityHandler(pg storage.PostgresStore, ch storage.ClickHouseStore, redis storage.RedisCache) *LoggingActivityHandler {
	return &LoggingActivityHandler{pg: pg, ch: ch, redis: redis}
}

func (h *LoggingActivityHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	data, err := getOrComputeLoggingActivity(r.Context(), h.redis, tenantID, jobID.String())
	if err != nil {
		api.Error(w, http.StatusInternalServerError, api.ErrCodeInternalError, "logging activity data not available")
		return
	}
