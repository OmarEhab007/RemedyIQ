...
func TestNewRootCauseSkill(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
...
func TestRootCauseSkill_Name(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
	assert.Equal(t, "root_cause", skill.Name())
}

func TestRootCauseSkill_Description(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
...
func TestRootCauseSkill_Examples(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
...
func TestRootCauseSkill_ImplementsSkillInterface(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
	var _ ai.Skill = skill
}

func TestRootCauseSkill_Execute_Validation(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
...
func TestRootCauseSkill_Execute_FallbackNilClient(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)

	input := ai.SkillInput{
		Query:    "What is the root cause of these anomalies?",
		JobID:    "test-job-id",
	...
	assert.Equal(t, "root_cause", output.SkillName)
	assert.Zero(t, output.TokensUsed)
	assert.Zero(t, output.LatencyMS)
}

func TestRootCauseSkill_Execute_FallbackUnavailableClient(t *testing.T) {
	var nilClient *ai.Client
	skill := NewRootCauseSkill(nilClient, nil)

	input := ai.SkillInput{
		Query:    "Why are multiple operations slow at the same time?",
		JobID:    "test-job-id",
		TenantID: "test-tenant-id",
	}
...
func TestRootCauseSkill_FetchRootCauseContext_NilClickHouse(t *testing.T) {
	skill := NewRootCauseSkill(nil, nil)
	ctx := context.Background()

	result := skill.fetchRootCauseContext(ctx, "tenant-1", "job-1")
	assert.Contains(t, result, "ClickHouse not configured")
}

func TestRootCauseSkill_Execute_AIQueryError_ReturnsFallback(t *testing.T) {
	client, err := ai.NewClient("sk-ant-test-invalid-key", "test-model")
	require.NoError(t, err)
	require.True(t, client.IsAvailable())

	skill := NewRootCauseSkill(client, nil)
