...
func TestNewNLQuerySkill(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
...
func TestNLQuerySkill_Name(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
	assert.Equal(t, "nl_query", skill.Name())
}

func TestNLQuerySkill_Description(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
...
func TestNLQuerySkill_Examples(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
...
func TestNLQuerySkill_ImplementsSkillInterface(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
	var _ ai.Skill = skill
}

func TestNLQuerySkill_Execute_Validation(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			input: ai.SkillInput{
				Query: "What were the slowest API calls?",
				JobID: "job-1",
			},
		...
		},
		{
			name: "missing job_id",
			input: ai.SkillInput{
				Query:    "Show me all failed SQL queries",
				TenantID: "tenant-1",
			},
...
func TestNLQuerySkill_Execute_FallbackNilClient(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
...
func TestNLQuerySkill_Execute_FallbackUnavailableClient(t *testing.T) {
	var nilClient *ai.Client
	skill := NewNLQuerySkill(nilClient, nil)
...
func TestNLQuerySkill_FetchLogContext_NilClickHouse(t *testing.T) {
	skill := NewNLQuerySkill(nil, nil)
...
func TestNLQuerySkill_Execute_AIQueryError_ReturnsFallback(t *testing.T) {
	client, err := ai.NewClient("sk-ant-test-invalid-key", "test-model")
	require.NoError(t, err)
	require.True(t, client.IsAvailable())

	skill := NewNLQuerySkill(client, nil)
